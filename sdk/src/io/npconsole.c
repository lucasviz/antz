...
void npConsoleMenuText( pNPconsole console, void* dataRef )
{
	...
	/*! Exit console */
	if( !strncmp(input, "exit", 4) || !strncmp(input, "EXIT", 4) )
	{
...
/*	else if( !strncmp(input, "refresh", 7) || !strncmp(input, "REFRESH", 7) )
	...
	else if( !strncmp(input, "set ", 4) || !strncmp(input, "SET ", 4) )
	{
		input += 4;

		//dbName = npdbActiveDB( data );
		if( !npdbActiveDB(data) )
		{
			npPostMsg( "No Active DB - LOAD, SAVE or USE to set Active DB", kNPmsgView, data );
	...
	else if( !strncmp(input, "use ", 4) || !strncmp(input, "USE ", 4) )
	{
		...
		sprintf(msg,"USE Database %s", dbName);
		npPostMsg(msg, kNPmsgView, dataRef);
	//	err = npdbUse( dbList->list[itemChosen], dataRef );
		err = npdbUse( dbName, dataRef );
		if( err )
	...
	else if( !strncmp(input, "save", 4) || !strncmp(input, "SAVE", 4) )
	{
		...
		if( strlen(input) == 4 )
		{
			...
			if( !dbName )
				...
				npdbSaveUpdate( data );	//save update to active DB
		}
		else	//save with name
		{
			input +=5 ;
			...
			if( 0 )// npdbExists(input, data) )
				npdbSaveUpdate( dataRef );	//save update to active DB
			else
			{
			...
			//	if (itemChosen < 0 || itemChosen >  console->menu->count)
				...
				//handle saving to a DB item # and illegal DB names... starts with digit..etc.

				sprintf(msg,"Saving New Database %s", input );
				npPostMsg(msg, kNPmsgView, dataRef);
				err = npdbSaveAs( dbID, input, dataRef );
...
void npUpdateConsoleMenu (pNPconsole console, void* dataRef)
{
	...
	if( !dbName )
		...
		sprintf( msg, "Active DB: %s", dbName);//, hostName );
